namespace PowerShell.Storage.Queue
{
    ...
    [Cmdlet(VerbsCommon.Get, "AzureStorageQueueContent")]
    public class GetAzureStorageQueueContent : Cmdlet
    {
        ...
        [Parameter(Position = 3,
                   HelpMessage = "Lets you peek at the messages."
        )]
        ...
        protected override void ProcessRecord()
        {
            ...
            try
            {
                if (peek)
                {
                    WriteVerbose("Peeking messages from Azure Storage Queue.");

                    //32 is the limit of messages returned from queue with query
                    WriteObject(queue.PeekMessages(32), true);
                }
                else
                {
                    WriteVerbose("Getting messages from Azure Storage Queue and removing them.");

                    foreach (CloudQueueMessage message in queue.GetMessages(32))
                    {
                        WriteObject(message, true);
